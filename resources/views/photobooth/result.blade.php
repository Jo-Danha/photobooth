...
@section('title', 'Hasil Foto Anda - Photobooth')
    ...
    <div class="bg-slate-900 border border-slate-800 rounded-3xl p-6 sm:p-10 shadow-2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            ...
            <div class="flex justify-center bg-slate-950 p-4 rounded-2xl border border-slate-800 shadow-inner">
                <img id="mainResultImage" src="{{ asset($session->result_image_path) }}" alt="Hasil Foto Photobooth" class="max-h-[500px] rounded-lg shadow-2xl object-contain">
            </div>
            ...
            <div class="flex flex-col items-center sm:items-start space-y-6">
                ...
                <div class="w-full space-y-3">
                    ...
                    @if(isset($session->metadata['gdrive_link']))
                        <a href="{{ $session->metadata['gdrive_link'] }}" target="_blank" class="w-full py-3 px-6 rounded-xl bg-emerald-950/80 hover:bg-emerald-900 border border-emerald-700/60 text-emerald-300 font-semibold text-xs flex items-center justify-center gap-2 transition-all">
...
<script>
    ...
    function printOnlyPhoto() {
        ...
        printWindow.document.write(`
            ...
            <html>
                <head>
                    <title>Cetak Foto - {{ $session->order_id }}</title>
                    <style>
