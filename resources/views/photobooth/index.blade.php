...
@section('title', 'Choose Your Layout - Photobooth')
    ...
    <div class="max-w-6xl mx-auto px-4 py-8">
        ...
        <div class="text-center">
            <div class="flex justify-center mb-4 gap-3">
                @if($isManual)
                <a href="{{ route('photobooth.gallery') }}" class="text-xs font-bold text-slate-300 px-3 py-1.5 rounded-full bg-slate-800 border border-slate-700 hover:bg-slate-700 flex items-center gap-2">
                ...
                <span class="text-[10px] font-bold uppercase tracking-widest text-brand-400 bg-brand-950/80 border border-brand-800/60 px-3 py-1.5 rounded-full">
                    @if($isManual) Wedding Mode — Gratis @elseif($isLocked) Template Dikunci Operator @else Langkah 1 dari 3: Pilih Template @endif
                </span>
                @if($isManual)
        ...
        <form action="{{ route('photobooth.session.create') }}" method="POST">
            @csrf
            @if($isLocked)
            <input type="hidden" name="layout_type" value="{{ $checkedLayout }}">
            @endif

            <!-- Layout Slideshow (persis photobooth-io.cc: tombol kiri/kanan + scroll horizontal tersembunyi) -->
            <div class="layout-nav-wrapper {{ $layoutDisplayMode === 'grid' ? 'grid-mode' : '' }} {{ $isLocked ? 'is-locked' : '' }}" data-size="{{ $layoutDisplaySize }}" id="layoutNavWrapper">
                ...
                <div class="layout-scroll-container" id="layoutScrollContainer">
                    <div id="layout-settings">
                        ...
                        @endphp
                        <div class="layout-contents" data-layout-id="{{ $pkg['id'] }}">
                            <input type="radio" name="layout_type" id="layout_{{ $pkg['id'] }}"
                                   value="{{ $pkg['id'] }}" class="sr-only layout-radio"
                                   data-default="{{ $index === 0 ? '1' : '0' }}"
                                   {{ ($index === 0) ? 'checked' : '' }}
                                   {{ $isLocked ? 'disabled' : '' }}>
                            <label for="layout_{{ $pkg['id'] }}" class="layout-holder {{ $pkg['popular'] ? 'badge-popular' : '' }} {{ $isLocked ? 'is-locked' : '' }}">
                                <div class="{{ $wrapper }}">
                                    <img src="{{ asset('layout-previews/'.$pkg['id'].'.png') }}"
                                         class="{{ $isNew ? 'special-layout-img' : 'layout-img' }}"
                                         alt="{{ $pkg['name'] }}" loading="lazy" decoding="async"/>
                                </div>
                                <span class="check-badge"><i class="fa-solid fa-check"></i></span>
                            </label>
                            <h2 class="layout-label">{{ $pkg['name'] }}</h2>
                            <div>
                                <p class="layout-description">{{ $sizeLabel }}</p>
                                <p class="layout-description" style="font-weight:700;color:{{ $isManual ? '#34d399' : ($setting->is_payment_enabled ? '#fff' : '#34d399') }};">{{ $price }} · {{ $pkg['duration'] }} menit</p>
                            </div>
                        </div>
                        @endforeach
                    </div>
            ...
            @elseif(!$setting->is_payment_enabled)
            <div class="flex justify-center mt-4">
                <div class="w-full sm:w-96 bg-slate-900/80 border border-slate-800 rounded-2xl p-4 flex flex-col items-center gap-2">
                    ...
                    @if($setting->event_voucher_code)
                    <input type="text" name="voucher_code" placeholder="Masukkan kode voucher (opsional)" value="{{ $setting->event_voucher_code }}" class="w-full bg-slate-950 border border-slate-700 text-slate-200 text-xs rounded-xl px-3 py-2.5 text-center uppercase font-mono focus:border-brand-500">
                    @else
            ...
            <div class="flex justify-center mt-5">
                <button type="submit" class="w-full sm:w-auto px-10 py-4 rounded-xl bg-gradient-to-r from-brand-600 to-indigo-600 hover:from-brand-500 hover:to-indigo-500 text-white font-bold text-base shadow-lg shadow-brand-500/25 flex items-center justify-center gap-3 transition-all active:scale-95">
                    <span>@if($isManual) Mulai Foto Gratis @elseif($setting->is_payment_enabled) Lanjut @else Mulai Foto Gratis @endif</span>
...
<script>
    ...
    const autoScroll = {{ $autoScroll ? 'true' : 'false' }};
    const autoScrollInterval = {{ $autoScrollInterval }};
