...
<body class="bg-slate-950 text-slate-100 min-h-screen flex flex-col justify-between p-4 selection:bg-pink-500">
    <div class="max-w-md mx-auto w-full text-center my-auto py-6">
        ...
        <p class="text-xs text-slate-400 mt-1 mb-6">Order ID: <?php echo e($session->order_id); ?></p>

        <div class="bg-slate-900 p-3 rounded-2xl border border-slate-800 shadow-2xl mb-6 inline-block">
            <img id="mobilePhoto" src="/file/<?php echo e($session->session_token); ?>" alt="Photobooth" class="max-h-[55vh] rounded-lg mx-auto shadow-md">
        </div>

        <div class="space-y-3">
            <a id="btnManualDownload" href="/file/<?php echo e($session->session_token); ?>" download="Photobooth_<?php echo e($session->order_id); ?>.png" class="w-full py-4 rounded-xl bg-gradient-to-r from-pink-600 to-indigo-600 text-white font-bold text-sm shadow-xl flex items-center justify-center gap-2 active:scale-95 transition-all">
            ...
            <?php if(isset($session->metadata['gdrive_link'])): ?>
                <a href="<?php echo e($session->metadata['gdrive_link']); ?>" target="_blank" class="w-full py-3.5 rounded-xl bg-emerald-950 border border-emerald-700 text-emerald-300 font-semibold text-xs flex items-center justify-center gap-2">
                    <i class="fa-brands fa-google-drive"></i>
                    <span>Buka File Asli di Google Drive</span>
                </a>
            <?php endif; ?>

            <?php if(isset($setting) && $setting->enable_email): ?>
    ...
    <footer class="text-center text-[11px] text-slate-600 pb-2">
        Photobooth Studio System &copy; <?php echo e(date('Y')); ?>
    ...
    <script>
        window.addEventListener('DOMContentLoaded', () => {
            const photoUrl = "/file/<?php echo e($session->session_token); ?>";
            const filename = "Photobooth_<?php echo e($session->order_id); ?>.png";
        ...
        <?php if($setting->enable_email): ?>
            ...
            if (btnSendEmail) {
                btnSendEmail.addEventListener('click', async () => {
                    ...
                    try {
                        const res = await fetch("/email/<?php echo e($session->session_token); ?>", {
                            method: 'POST',
                            headers: {
...
</html><?php /**PATH D:\xampp\htdocs\photobooth\resources\views\photobooth\download.blade.php ENDPATH**/ ?>