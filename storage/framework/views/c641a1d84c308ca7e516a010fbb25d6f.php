<?php $__env->startSection('title', 'Pembayaran - Photobooth'); ?>
...
<div class="max-w-md mx-auto px-4 py-8">
    <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-2xl text-center">
        ...
        <div class="flex gap-2 mt-5" id="methodTabs">
            ...
            <button type="button" data-method="<?php echo e($m); ?>"
                class="method-tab flex-1 py-2.5 rounded-xl text-xs font-bold border transition-all
                <?php echo e(strtolower($session->payment_method) === $m ? 'bg-brand-600 border-brand-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-600'); ?>">
                <?php if($m == 'qris'): ?><i class="fa-solid fa-qrcode mr-1"></i> QRIS
                <?php elseif($m == 'cash'): ?><i class="fa-solid fa-money-bill-wave mr-1"></i> Tunai
                <?php elseif($m == 'transfer'): ?><i class="fa-solid fa-building-columns mr-1"></i> Transfer
                <?php endif; ?>
            </button>
        ...
        <!-- Panel QRIS -->
        <div id="panel-qris" class="method-panel mt-5 <?php echo e(strtolower($session->payment_method) == 'qris' ? '' : 'hidden'); ?>">
            <div class="my-4 p-4 bg-white rounded-2xl inline-block shadow-lg">
                <img id="qrisImg" src="<?php echo e($qrUrl); ?>" alt="QRIS Code" class="w-64 h-64 mx-auto rounded-lg">
        ...
        <!-- Panel Tunai -->
        <div id="panel-cash" class="method-panel mt-5 <?php echo e(strtolower($session->payment_method) == 'cash' ? '' : 'hidden'); ?>">
            ...
            <?php if($bankAccount): ?>
            <p class="text-[11px] text-slate-500 mt-2"><?php echo e($bankAccount); ?></p>
            <?php endif; ?>
        </div>

        <!-- Panel Transfer -->
        <div id="panel-transfer" class="method-panel mt-5 <?php echo e(strtolower($session->payment_method) == 'transfer' ? '' : 'hidden'); ?>">
            ...
            <div class="mt-2 p-3 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 font-mono">
                <?php echo e($bankAccount ?: 'Hubungi operator booth untuk info rekening.'); ?>
        ...
        <div class="bg-slate-950/80 rounded-xl p-4 text-left text-xs space-y-2 border border-slate-800/80 mt-5">
            <div class="flex justify-between"><span class="text-slate-400">Order ID</span><span class="font-mono font-semibold text-slate-200"><?php echo e($session->order_id); ?></span></div>
            <div class="flex justify-between"><span class="text-slate-400">Pilihan Paket</span><span class="font-semibold text-slate-200"><?php echo e($session->package_name); ?></span></div>
            <div class="flex justify-between"><span class="text-slate-400">Durasi Sesi Foto</span><span class="font-semibold text-emerald-400"><?php echo e($session->duration_minutes); ?> Menit</span></div>
            <div class="pt-2 border-t border-slate-800 flex justify-between text-sm">
                ...
                <span class="font-extrabold text-brand-400">Rp <?php echo e(number_format($session->amount, 0, ',', '.')); ?></span>
            </div>
        </div>

        <div id="waitingBox" class="flex items-center justify-center gap-2 text-xs text-slate-400 my-5 <?php echo e(strtolower($session->payment_method) == 'qris' ? '' : 'hidden'); ?>">
        ...
        <form action="<?php echo e(route('photobooth.simulate.pay', ['token' => $session->session_token])); ?>" method="POST" class="cash-confirm <?php echo e(strtolower($session->payment_method) == 'qris' ? 'hidden' : ''); ?>">
            <?php echo csrf_field(); ?>
            <button type="submit" class="w-full py-3 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-sm shadow-lg transition-all flex items-center justify-center gap-2">
                ...
                <span><?php echo e(strtolower($session->payment_method) == 'cash' ? 'Saya Sudah Bayar (Tunai)' : 'Saya Sudah Transfer'); ?></span>
            </button>
        </form>

        <!-- Simulasi bayar untuk QRIS (demo/testing) -->
        <?php if(strtolower($session->payment_method) == 'qris'): ?>
        <form action="<?php echo e(route('photobooth.simulate.pay', ['token' => $session->session_token])); ?>" method="POST" class="mt-3">
            <?php echo csrf_field(); ?>
...
<script>
    ...
    methodTabs.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            showPanel(method);
            fetch("<?php echo e(route('photobooth.checkout.method', ['token' => $session->session_token])); ?>", {
                method: 'POST',
    ...
    const statusUrl = "<?php echo e(route('photobooth.checkout.status', ['token' => $session->session_token])); ?>";
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\xampp\htdocs\photobooth\resources\views\photobooth\checkout.blade.php ENDPATH**/ ?>